    ...
    with gfile.GFile(data_path, mode="rb") as f:
      ...
      for line in f:
        ...
        for w in tokens:
          ...
          if word in vocab:
            vocab[word] += 1
          elif word!=_COS and word!=_COQ:
            vocab[word] = 1
      vocab_list = _START_VOCAB + sorted(vocab, key=vocab.get, reverse=True)
      if len(vocab_list) > max_vocabulary_size:
        vocab_list = vocab_list[:max_vocabulary_size]
      vocab_list = vocab_list + _ENTITY_VOCAB   ###NEW
  ...
  with gfile.GFile(tokenized_dialog_path, mode="r") as fh:
      ...
      while source and target and (not max_size or counter < max_size):
        ...
        if counter % 100000 == 0:
          ...
          sys.stdout.flush()
        source_temp = source.split('-2')
        story_ids = [[int(x) for x in story_temp.split()] for story_temp in source_temp[0].split('-1')]
        query_ids = [int(x) for x in source_temp[1].split()]
        #source_ids = [int(x) for x in source.split()]
        target_ids = [int(x) for x in target.split()]
        target_ids.append(EOS_ID)

        for bucket_id, (source_size, target_size) in enumerate(buckets):
          if len(query_ids) < source_size and len(target_ids) < target_size:
            data_set[bucket_id].append([story_ids, query_ids, target_ids])
            break
        source, target = fh.readline(), fh.readline()
  return data_set
