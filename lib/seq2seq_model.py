...
class Seq2SeqModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(self.scope_name):
      ...
      def seq2seq_f(story_inputs, encoder_inputs, decoder_inputs, feed_previous):
        return tf_seq2seq.embedding_attention_entity_seq2seq(
            story_inputs, 
            encoder_inputs, 
            decoder_inputs, 
            self.batch_size,
  ...
  def step(self, session, story_inputs, encoder_inputs, decoder_inputs, target_weights, bucket_id, forward_only, force_dec_input=False, advantage=None):
    ...
    input_feed[self.story_inputs.name] = story_inputs
  ...
  def step_rf(self, args, session, encoder_inputs, decoder_inputs, target_weights, bucket_id, rev_vocab=None, debug=True):
  ...
  def get_batch(self, data, bucket_id):
    ...
    encoder_size, decoder_size = self.buckets[bucket_id]
    story_inputs, encoder_inputs, decoder_inputs = [], [], []

    # Get a random batch of encoder and decoder inputs from data,
    # pad them if needed, reverse encoder inputs and add GO to decoder.
    all_story_inputs = []
    for _ in xrange(self.batch_size):
      all_story_input, encoder_input, decoder_input = random.choice(data[bucket_id])
      all_story = []
      for story in all_story_input:
        if len(story)>story_size:
          all_story.append(story[:story_size])
        else:
          all_story.append(story + [data_utils.PAD_ID] * (story_size - len(story))) 
      all_story_inputs.append(all_story)
    ...
    batch_story = np.array( all_story_inputs, dtype=np.int32 )
    ...
    return batch_story, batch_encoder_inputs, batch_decoder_inputs, batch_weights
