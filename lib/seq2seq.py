...
def embedding_attention_entity_seq2seq(story_inputs,
                                encoder_inputs,
                                decoder_inputs,
                                batch_size,
                                cell,
  ...
  with variable_scope.variable_scope(
      scope or "embedding_attention_entity_seq2seq", dtype=dtype) as scope:
    dtype = scope.dtype
    # Embedding 
    embedding_params = tf.get_variable(name='embedding_params', shape=[num_encoder_symbols, embedding_size])
    embedding_mask = tf.constant(value=[0 if i == 0 else 1 for i in range(num_encoder_symbols)], shape=[num_encoder_symbols, 1], dtype=tf.float32)
    ...
    params = {
      'key_number': 30,
      'vocab_size': num_encoder_symbols,
    ...
    encoder_outputs, encoder_state = core_rnn.static_rnn(
        cell, encoder_embedding_inputs, dtype=dtype)

    hidden = []
    for e in encoder_outputs:
      hidden.append( tf.concat((entity_vector, e), axis=1) )
    ...
    if output_projection is None:
      ...
      output_size = num_decoder_symbols
    if isinstance(feed_previous, bool):
      outputs, state = entity_embedding_attention_decoder(
          decoder_inputs,
          encoder_state,
          attention_states,
          cell,
...
def entity_embedding_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
      scope or "entity_embedding_attention_decoder", dtype=dtype) as scope:
...
def one2many_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "one2many_rnn_seq2seq", dtype=dtype) as scope:
    dtype = scope.dtype
...
def model_with_buckets(story_inputs,
                       encoder_inputs,
                       decoder_inputs,
                       targets,
                       weights,
